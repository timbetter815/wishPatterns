...
/**
 * TODO 工厂方法模式：定义了一个创建对象的接口，但是有子类决定要实例化的类是哪一个。工厂方法把类的实例化推迟到子类。
 * 1、定义一个抽象工厂方法类，所有的具体工厂类（AppleFactory、BananaFactory等）继承该类
 ...
 *   -- 示例2：java.util.List 中的listIterator方法为工厂方法，所有的继承自List类的具体类，必须提供listIterator工厂方法，返回一个迭代对象列表，其中java。util.List为抽象工厂类
